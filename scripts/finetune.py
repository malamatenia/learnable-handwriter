...
def finetune(trainer, max_epochs, save_sprites_dir, reconstructions_path, name, invert_sprites):
    ...
    for i in trange(max_epochs):
        ...
        for x in trainer.train_loader[0]:
            trainer.model.train()
            trainer.optimizer.zero_grad() #redefinir mon propre optimizer / freeze les params pour transformation et garder uniquement pour la selection
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Helper script to finetune Textualis on a specific script and generate figures.')
    ...
    parser.add_argument('-a', "--annotation_file", required=False, default=join(LIB_PATH, 'datasets/south_north_textualis/annotation.json'), type=str)
    parser.add_argument("--max_epochs", required=False, default=150, type=int)
    parser.add_argument("--mode", choices=["all", "sprites", "sprites-yannis"], default='sprites')
